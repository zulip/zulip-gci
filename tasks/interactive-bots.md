...
* Create a `~/.zuliprc-local` file with credentials (`nano ~/.zuliprc-local`):
  ```
  [api]
  key=<api-key> (Of the bot)
  email=<email> (Of the bot)
  site=<dev-url> (localhost/Zulip server)
  ```
...
* Take screenshots showing that the bot is working. Make sure to take screenshots
of:

 - the output in your terminal window with the bot running, including the command you used to
 run the bot and the output with the bot description
 - messages sent to the bot by you in the `devel` and `social` streams
...
filenames do not use spaces. Instead, use dashes (`-`).

* Note down any places you got stuck, problems or errors you ran into while
completing the task. Add your notes as a `notes.md` file to
